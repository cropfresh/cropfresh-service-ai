        ...
        try {
          ...
          handler(call, (err: any, value?: any, trailer?: grpc.Metadata, flags?: number) => {
            ...
            try {
              ...
              grpcRequestDuration.labels('ai-service', methodName, statusCode).observe(duration);
              grpcRequestTotal.labels('ai-service', methodName, statusCode).inc();
            } catch (metricsErr) { }
          ...
          try {
            const { grpcRequestDuration, grpcRequestTotal } = require('../middleware/monitoring');
            const duration = (Date.now() - startTime) / 1000;
            grpcRequestDuration.labels('ai-service', methodName, 'INTERNAL').observe(duration);
            grpcRequestTotal.labels('ai-service', methodName, 'INTERNAL').inc();
          } catch (metricsErr) { }
